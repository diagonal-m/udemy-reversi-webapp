...
export class TurnService {
  ...
  async registerTurn(turnCount: number, disc: number, x: number, y: number) {
    ...
    try {
      ...
      const previousTurnRecord = await turnGateway.findForGameIdAndTurnCount(conn, gameRecord.id, previoustTurnCount)
      ...
      // ターンを保存する
      const turnRecord = await turnGateway.insert(conn, newTurn.gameId, newTurn.turnCount, newTurn.nextDisc, newTurn.endAt)
